...
import {
  CART_OPEN_STATUS,
} from "../../utility/Actions/cartAction/types";
...
class MyCart extends React.Component {
  ...
  changeFocus = e => {
    if (
      ...
      e.keyCode == 9 &&
      e.target.classList.value.indexOf("cart__btn") > -1
    ) {
      e.preventDefault();
  ...
  componentDidMount() {
    ...
    if (this.mainRef.current && this.closeRef.current) {
      this.mainRef.current.tabIndex = 0;
      this.mainRef.current.focus();
      this.closeRef.current.tabIndex = -1;
    }
  }
  ...
  render() {
    return (
      <main className="cart" ref={this.mainRef}>
        {this.state.cart.length ? (
          <React.Fragment>
            <div>
              <div className="cart__desc">
                ...
                {this.state.isModalOpen && (
                  <button
                    ...
                    aria-label={labelConfig.CloseCartButton}
                    aria-controls={labelConfig.CloseCart}
              ...
              <div className="cart__banner">
                <img
                  alt={labelConfig.CartBanner}
                  src={urlConfig.cartbannerImageUrl}
                  className="cart__banner__img"
                />
            ...
            <div className="cart__checkout">
              ...
              <CustomButton
                ...
                styleClass="cart__btn cart__btn--full"
                label={labelConfig.CartCheckout}
                control={labelConfig.CartPayment}
                rightText={
                  labelConfig.Rupee +
                  this.state.total +
                  " " +
                  labelConfig.RightArrow
                }
                text={labelConfig.CartProceed}
              />
            </div>
          </React.Fragment>
        ) : (
          <React.Fragment>
           ...
           <div>
            ...
            <CustomButton
              handler={this.redirectHome}
              styleClass="cart__btn cart__btn--empty"
              label={labelConfig.CartEmptyButtonText}
              control={labelConfig.CartEmptyButtonText}
              text={labelConfig.CartEmptyButtonText}
            />
          </React.Fragment>
        )}
      </main>
...
export default withRouter(
  connect(mapStatetoProps, mapDispatcherToProps)(MyCart)
);
